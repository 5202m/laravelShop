...
class CreateCategoryTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('category', function (Blueprint $table) {
            ...
            $table->string('style', 150)->default('')->comment('分类的样式表文件');
            ...
            $table->string('top_style_tpl', 255)->default('')->comment('顶级分类页模板');
            $table->string('style_icon', 50)->default('other')->comment('自定义分类菜单图标');
            $table->string('cat_icon', 255)->default('')->comment('分类菜单图标');
            $table->tinyInteger('is_top_show')->unsigned()->default(0)->comment('');
            $table->text('category_links')->nullable()->comment('分类跳转链接');
            $table->text('category_topic')->nullable()->comment('分类树顶级分类模块内容');
            $table->text('pinyin_keyword')->nullable()->comment('拼音关键字');
            $table->string('cat_alias_name', 90)->default('')->comment('分类别名');
            $table->smallInteger('commission_rate')->unsigned()->default(0)->comment('');
            $table->string('touch_icon', 255)->default('')->comment('手机小图标');
            //$table->timestamps();
        });
    }
